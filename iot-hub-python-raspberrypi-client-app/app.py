...
#MSG_TXT = "{\"messageId\": %d,\"deviceId\": \"Raspberry Pi - Python\",\"temperature\": %f,\"humidity\": %f}"
MSG_TXT = "{\"messageId\": %d,\"deviceId\": \"SenseHatRPi\",\"temperature\": %f,\"humidity\": %f,\"temperatureAlert\": \"%s\",\"humidityAlert\": \"%s\"}"
    ...
    try:
        ...
        while True:
            ...
            if MESSAGE_SWITCH:
                ...
                TempMessage2sense= str("{:.2f}".format(temperature))
		HumMessage2sense= str("{:.2f}".format(humidity))
		start_new_thread(showValuesOnSenseHat,(sensor,TempMessage2sense,HumMessage2sense,))
                #sensor.show_message(message2sense,0.1,text_colour=[255,255,255],back_colour=[0,0,0])
		msg_txt_formatted = MSG_TXT % (MESSAGE_COUNT+1,temperature,humidity,"true" if temperature > config.TEMPERATURE_ALERT else "false","true" if humidity > config.HUMIDITY_ALERT else "false")
...
def showValuesOnSenseHat(sensor,TempMessage2sense,HumMessage2sense):
    sensor.show_message("Temp:"+TempMessage2sense,0.1,text_colour=[255,255,255],back_colour=[0,0,0])
    sensor.show_message("Hum:"+HumMessage2sense,0.1,text_colour=[255,255,255],back_colour=[0,0,0])
